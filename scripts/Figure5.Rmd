...
dir_name<-"/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/projects/BMS153/data/model_bootstrap70"
dir_name<-"Checkmat153_Immunogenicity_NatureMedicine/data/model_bootstrap70"
...
# data_dir<-"/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/Input/AutoFilt/MachineLearning_v2/data/model_outputs/integrated_model/bootstrap_50runs_70"
# cf<-read.delim(paste0(data_dir,"/Coefficients_avg.txt"),sep="\t",header = TRUE)
# wh = which(fit$lambda==fit$lambda.min)
# fit$glmnet.fit$beta[,wh] = cf[2:67,50]
...
varImpdf$seqPos<-NA
aaindex_def<-read.delim("/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/Input/AutoFilt/MachineLearning_v2/data/model_outputs/integrated_model/TTN_iFeatureTTN_noUnivariate_lasso_0/AAINDEX_key.txt",header = FALSE, sep = "|",quote="")
...
base_dir<-"/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/projects/BMS153/data/model_bootstrap70/"
dir_name<-"/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/projects/BMS153/data/model_bootstrap70/"
...
# score_file<-"/prediction_scores.txt"
# tesla_score_file<-"/val_prediction_scores_tesla.txt"
# study_score_file<-"/val_prediction_scores_study.txt"
...
# wilcox.test(score$s1, test_data$tet_pos_or_neg)
# wilcox.test(tesla_score$s1, tesla_data$tet_pos_or_neg)
# wilcox.test(study_score$s1, study_data$tet_pos_or_neg)

score$label<-test_data$tet_pos_or_neg
# score$label[which(score$label == 0)]<-"Non-Immunogenic"
# score$label[which(score$label == 1)]<-"Immunogenic"
...
# tesla_score$label[which(tesla_score$label == 0)]<-"Non-Immunogenic"
# tesla_score$label[which(tesla_score$label == 1)]<-"Immunogenic"
...
# study_score$label[which(study_score$label == 0)]<-"Non-Immunogenic"
# study_score$label[which(study_score$label == 1)]<-"Immunogenic"
...
# pdf(paste0(dir_name,"/output/Figure5C.pdf"),width=5,height=5)
# print(test_plot)
# dev.off()
# pdf(paste0(dir_name,"/output/Figure5D.pdf"),width=5,height=5)
# print(tesla_plot)
# dev.off()
# pdf(paste0(dir_name,"/output/Figure5E.pdf"),width=5,height=5)
# print(study_plot)
# dev.off()
```
...
#prediction<-read.delim("/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/Input/AutoFilt/MachineLearning_v2/data/model_outputs/integrated_model/bootstrap_50runs_70/predictions_m50/prediction_scores.txt",header = TRUE,sep="\t")
prediction<-read.delim("/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/projects/BMS153/data/model_bootstrap70/output/prediction_scores.txt",header = TRUE,sep="\t")
master_CCF<-read.delim("/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/Input/createMaster/data/master_NDT_VMResults_v2_CCFmean.txt",header = TRUE, sep = "\t")
...
#master_CCF_prediction<-merge(master_CCF,rank_prediction,by.x="PT_PEP",by.y="test.pt_pep",all.x=TRUE)
master_CCF_prediction<-merge(master_CCF,rank_prediction,by.x="PT_PEP",by.y="test_data.pt_pep",all.x=TRUE)
#test_data_CCF_predictions<-master_CCF_prediction[which(!is.na(master_CCF_prediction$s1)),]
...
#master_nonBinders<-master[which(master$Binder == "NON_BINDER"),]
#master_nonBinders$deltaCCF<-master_nonBinders$ccf_on - master_nonBinders$ccf_pre
...
#pdf("/Volumes/chant2lab/ChanLab_Common_Drive/Prerana/Input/AutoFilt/MachineLearning_v2/data/model_outputs/integrated_model/bootstrap_50runs_70/predictions_m50/cluster_plots/DeltaCCF_all_paired_variants_clusters_rmSanquin.pdf")
